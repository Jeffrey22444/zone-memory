...
- After every Acceptance pass, `验收区` must write a concise verdict for the same task ID to `docs/project_notes/issues.md`; only `通过` updates `Latest accepted slice` in its Current Summary.
